...
For outreach invocations, load only: `SOUL.md` + `AGENTS.md` (system prompt), this file (rules), and `MEMORY.md` (state). Minimal token load.
...
Set up Schedule Trigger nodes:

| Trigger | Cron | Purpose |
|---|---|---|
| Morning check-in | `0 8 * * *` | Check daily note / say good morning |
| Midday casual | `0 12 * * *` | Random casual check-in (50% chance, see below) |
...
| Random ping | Random interval node | Pure social check-in (see casual triggers) |

For the random/casual triggers: use an n8n Code node that generates a random number and only continues the workflow if it falls below the threshold (e.g., 50% chance). This prevents the bot from feeling predictable.

### How n8n triggers map to this file

Each n8n Schedule Trigger fires with a payload like `{ "trigger": "morning_checkin" }`. The workflow:
1. Reads `SOUL.md` + `AGENTS.md` + `MEMORY.md` + this file
2. Passes them to the AI Agent node along with the trigger name
3. The agent finds the matching trigger section below and executes
4. The agent does NOT need to decide what files to load — the workflow handles it

For the random/casual triggers, the n8n Code node rolls `Math.random()` before the AI Agent node. If the roll fails, the workflow stops — the agent is never invoked. This keeps token costs zero for skipped triggers.
